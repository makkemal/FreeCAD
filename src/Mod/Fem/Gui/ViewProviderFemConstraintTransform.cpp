...
void ViewProviderFemConstraintTransform::updateData(const App::Property* prop)
{
    ...
    if (strcmp(prop->getName(),"Points") == 0) {
        ...
        for (std::vector<Base::Vector3d>::const_iterator p = points.begin(); p != points.end(); p++) {
            ...
            base = base + dirz * scaledlengthA;
            m.setTransform(base, rot, SbVec3f(1,1,1));
            matrices[id] = m;
            id++;

            basex = basex + dirx * scaledlengthA;
            mx.setTransform(basex, rotx, SbVec3f(1,1,1));
            matrices[idy] = mx;
            idx++;

            basey = basey + diry * scaledlengthA;
            my.setTransform(basey, roty, SbVec3f(1,1,1));
            matrices[idy] = my;
            idy++;
#else
